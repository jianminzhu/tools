 ...
 */
var ReactMount = {
  rjc:function(nextDescriptor, jsel, callback) {
	try {
	  /*渲染多个jquery对象*/
	  $(function(){ 
		  var jitems=null;
		  if(jsel instanceof jQuery){
			  jitems =jsel;
		  }else{
			  jitems=$(jsel);
		  } 
		  jitems.each(function(){ 
			  ReactMount.renderComponent(nextDescriptor,this,callback);
		  })
	  })
	} catch (e) {
		alert("(jQuery notdefined) or (jsel mustbe a css selector or a dom)"+e.toString())
	}
  },
